...
static int running_all(void) {
  ...
  if (greatest_info.test_filter != NULL || greatest_info.suite_filter != NULL) {
    return 0;
  }
  return 1;
}
...
TEST just_fail(void) { FAIL(); }

#if defined(_MSC_VER) || defined(__MINGW32__)
static unsigned int seed_of_time(void) {
  static unsigned int counter = 1;
  unsigned int t = (unsigned int)(time(NULL) ^ clock());
  return t * counter++;
}
#else
...
SUITE(suite_failure) { RUN_TEST(just_fail); }
